...
class SavedBuilderAnalysis(Base):
  ...
  id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
  user_id: Mapped[int] = mapped_column(ForeignKey("users.id", ondelete="CASCADE"), index=True)
  # Same classification label as builder_keyword_catalog.classification (e.g. agri_prices)
...
class BuilderKeywordCatalog(Base):
  """Catalog rows for the analysis builder: classification, keyword_key, keyword_value."""
  ...
  id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
  classification: Mapped[str] = mapped_column(String(80), nullable=False, index=True)
