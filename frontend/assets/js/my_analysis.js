  ...
  document.addEventListener("DOMContentLoaded", function () {
    window.EtAuth.requireAuth({ redirect: true }).then(function (allowed) {
      ...
      function applyMetric(metricId, label, desc) {
        ...
        fetchJson("/api/builder/metric?keyword=" + encodeURIComponent(state.keyword) + "&metric=" + encodeURIComponent(state.metric), { method: "GET" })
          .then(function (data) {
      ...
      if (form) {
        form.addEventListener("submit", function (e) {
          ...
          fetchJson("/api/builder/suggestions?keyword=" + encodeURIComponent(state.keyword), { method: "GET" })
            .then(function (json) {
      ...
      try {
        ...
        if (kw) {
          ...
          state.keyword = kw;
          fetchJson("/api/builder/suggestions?keyword=" + encodeURIComponent(kw))
            .then(function (json) {
              var list = json && Array.isArray(json.suggestions) ? json.suggestions : [];
              renderSuggestions(list);
              if (sugPanel) sugPanel.hidden = false;
